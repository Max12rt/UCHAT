...
void mx_delete_chat(int id_chat) {
    ...
    const char *delete_chat = "DELETE FROM chats WHERE chat_id = ?1";
    
    error = sqlite3_prepare_v2(db, delete_chat, -1, &stmt, NULL);
    prinerr_ok(error, "Error preparing delete chat: ", sqlite3_errmsg(db), db);
   
    error = sqlite3_bind_int(stmt, 1, id_chat); //id chat
    prinerr_ok(error, "Error binding chat name parameter: ", sqlite3_errmsg(db), db);
    
    error = sqlite3_step(stmt);
    printerr_done(error, "Error executing delete chat: ", sqlite3_errmsg(db), db);
